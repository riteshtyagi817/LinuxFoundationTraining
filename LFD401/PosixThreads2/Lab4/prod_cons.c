...
void produce(int j){
	...
	printf(" PRODUCE : loc = %3d , id = %3d\n",location, (int) pthread_self() % 1000);
...
void *producer(void *arg){
	...
	while(j_produced  < nmax){
		...
		j_produced++;
		j_active++; 
		printf("prod  = %3d , active = %3d, id  = %3ld\n",j_produced, j_active, pthread_self() % 1000);
		produce(j_produced);
...
void *consumer(void *){
	...
	while(j_consumed  < nmax){
		...
		CON_CHECK;
		j_consumed++;
		printf("cons %3d, active %3d , id = %3d",j_consumed, j_active,(int) pthread_self()%1000);
		consume(j_consumed);
		j_active--;
...
int main(int argc, char *argv[]){
	...
	/* Below once looks, ok, buffer allocation, threads creation and joining */
	pthread_t t_p[NPROD];
	pthread_t t_c[NCONS];
